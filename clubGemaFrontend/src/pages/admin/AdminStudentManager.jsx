...
import {
    Search, Phone, ShieldAlert, ChevronRight, ArrowLeft, Mail,
        ...
        try {
            ...
            if (response.ok) {
                const formattedData = result.data.map(user => {
                    const inscripciones = user.inscripciones || [];
                    ...
                    return {
                        ...
                        cumpleanos: user.fecha_nacimiento ? format(new Date(user.fecha_nacimiento.split('T')[0] + 'T05:00:00Z'), "dd 'de' MMM", { locale: es }) : 'S/D',
                        sedes: sedesNombres,
                                    ...
                                    <td className="p-8 text-center">
                                        <div className="flex flex-col items-center gap-1.5">
                                            <div className={`p-2.5 rounded-2xl transition-colors ${alum.cumpleanos !== 'S/D' ? 'bg-pink-50 text-pink-500' : 'bg-slate-50 text-slate-300'}`}>
                                                <Cake size={16} fill={alum.cumpleanos !== 'S/D' ? "currentColor" : "none"} />
                                            </div>
